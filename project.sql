...
) ENGINE=InnoDB AUTO_INCREMENT=45 DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_0900_ai_ci COMMENT='用户角色中间表'
...
CREATE TABLE `project_info` (
  ...
  `project_source` varchar(100) DEFAULT '5' COMMENT '项目来源，1=自拓非招投标，2=自拓+招投标，3=渠道维护，4=渠道维护+招投标，5=公开招投标',
...
CREATE TABLE `project_task` (
  ...
  `deliver_type` tinyint DEFAULT NULL COMMENT '工作成果类型 1=文档,2=表格,3=图片,4=其他',
...
CREATE TABLE `sys_message` (
  ...
  `msg_type` tinyint DEFAULT '1' COMMENT '消息类型 1=任务提醒,2=消息通知3=消息预警',
...
CREATE TABLE `project_extend` (
  ...
  `assess_type` tinyint DEFAULT NULL COMMENT '项目考核类型（ 1=报告书 ，7=报告表，8=登记表,2=场调,3=应急预案,4=验收,5=环评入围,6=场调入围,99=其他',
  `monitor_fee` decimal(16,4) DEFAULT NULL COMMENT '监测费',
  `review_fee` decimal(16,4) DEFAULT NULL COMMENT '评审费',
  `cooperation_fee` decimal(16,4) DEFAULT NULL COMMENT '协作费/业务费',
  `public_notice_fee` decimal(16,4) DEFAULT NULL COMMENT '公示公告费',
  `material_fee` decimal(16,4) DEFAULT NULL COMMENT '材料费',
  `travel_fee` decimal(16,4) DEFAULT NULL COMMENT '差旅费',
  `bind_express_fee` decimal(16,4) DEFAULT NULL COMMENT '装订费/快递费',
  `other_direct_fee` decimal(16,4) DEFAULT NULL COMMENT '其他直接费用',
  `is_government_approve` tinyint DEFAULT NULL COMMENT '是否通过政府部门审批或备案 0=否 1=是',
  `meeting_approve` tinyint DEFAULT NULL COMMENT '过会审批(0=一次性过会、1=多次过会、2=其他等)',
  `is_archive_finish` tinyint DEFAULT NULL COMMENT '存档是否完成 0=否 1=是',
  `progress_info` varchar(500) DEFAULT NULL COMMENT '进展情况',
...
CREATE TABLE `project_assessment` (
  ...
  `other_direct_fee` decimal(16,4) DEFAULT NULL COMMENT '其他直接费用',
  `net_receive_amount` decimal(16,4) DEFAULT NULL COMMENT '净收款额',
  `net_assess_amount` decimal(16,4) DEFAULT NULL COMMENT '净考核额',
  `is_government_approve` tinyint DEFAULT NULL COMMENT '是否通过政府部门审批或备案 0=否 1=是',
  `meeting_approve` tinyint DEFAULT NULL COMMENT '过会审批(0=一次性过会、1=多次过会、2=其他等)',
  `is_archive_finish` tinyint DEFAULT NULL COMMENT '存档是否完成 0=否 1=是',
  `progress_info` varchar(500) DEFAULT NULL COMMENT '进展情况',
  ...
  UNIQUE KEY `uk_project_id` (`project_id`)
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COMMENT='项目考核表';

CREATE TABLE `project_ratio` (
  `id` bigint NOT NULL AUTO_INCREMENT COMMENT '主键ID',
  `project_id` bigint NOT NULL COMMENT '关联项目ID(project_info.id)',
  `user_id` bigint NOT NULL COMMENT '用户ID',
  `ratio` decimal(5,2) DEFAULT NULL COMMENT '占比(%)，例如 30.50 代表30.50%',
  `create_time` datetime DEFAULT CURRENT_TIMESTAMP COMMENT '创建时间',
  PRIMARY KEY (`id`),
  UNIQUE KEY `uk_project_user` (`project_id`,`user_id`),
  KEY `idx_project_id` (`project_id`),
  KEY `idx_user_id` (`user_id`)
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COMMENT='项目分配表';

CREATE TABLE `project_income` (
  `id` bigint NOT NULL AUTO_INCREMENT COMMENT '主键ID',
  `project_id` bigint NOT NULL COMMENT '关联项目ID(project_info.id)',
  `receive_amount` decimal(14,2) DEFAULT NULL COMMENT '到账金额',
  `receive_time` date DEFAULT NULL COMMENT '到账时间',
  `payer_company_name` varchar(255) DEFAULT NULL COMMENT '付款单位名字',
  `pay_type` tinyint DEFAULT NULL COMMENT '付款方式 1=银承，2=转账',
  `is_matchnode` tinyint DEFAULT '0' COMMENT '是否与合同节点一致 0=否，1=是',
  `payment_node_id` bigint DEFAULT NULL COMMENT '收款节点id(关联sign_payment.id)',
  `create_time` datetime DEFAULT CURRENT_TIMESTAMP COMMENT '创建时间',
  PRIMARY KEY (`id`),
  KEY `idx_project_id` (`project_id`)
