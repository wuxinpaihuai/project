...
-- 项目费用明细表
CREATE TABLE `project_fee_info` (
  `id` bigint NOT NULL AUTO_INCREMENT COMMENT '主键ID',
  `project_id` bigint NOT NULL COMMENT '关联项目ID(project_info.id)',
  `fee_type` tinyint DEFAULT NULL COMMENT '费用类型（ 1=监测费/工程直接成本 ，2=评审费,3=协作费/业务费,4=公示公告费,5=材料费,6=差旅费,7=装订费/快递费,8=其他直接费用）',
  `fee_amount` decimal(16,4) DEFAULT NULL COMMENT '费用金额（元）',
  `fee_status` tinyint DEFAULT NULL COMMENT '费用状态（0=未提交，1=已提交）',
  `submit_user_id` bigint DEFAULT NULL COMMENT '报销人用户ID',
  `submit_user_name` varchar(50) DEFAULT NULL COMMENT '报销人姓名',
  `submit_user_phone` varchar(20) DEFAULT NULL COMMENT '报销人手机号',
  `remark` varchar(500) DEFAULT NULL COMMENT '备注',
  `create_time` datetime DEFAULT CURRENT_TIMESTAMP COMMENT '创建时间',
  `update_time` datetime DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP COMMENT '更新时间',
  PRIMARY KEY (`id`) 
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COMMENT='项目费用明细表';
